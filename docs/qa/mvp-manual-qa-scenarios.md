...
| --- | --- | --- | --- | --- | --- |
| QA-026 | Guest token cannot open authenticated app routes | Use request ID `QA-026-<timestamp>` and a valid guest public token as a credential header, not as an authenticated session cookie, to request `/platform` plus one project/admin route and API equivalents | Server-rendered protected pages return a login redirect with an encoded internal `next` path, while authenticated API routes return exact `401` or `403`; responses contain only generic auth/permission handling and no project, user, role, guest, payment, audit, or internal-note fields | request ID, response payload, route, and assertions from the negative-case table below | Public-token resolver and authenticated route middleware |
...
| --- | --- | --- | --- |
| QA-026 | Project, user, role, guest, RSVP, invitation, file, payment, and audit rows for the targeted project remain unchanged when queried by project ID and guest token ID | Timestamp-bounded `audit_logs` queries return zero route-side mutation rows; setup-only token creation audit rows from temporary fake test tokens must be identified separately and must not be counted as route-side access | Server-rendered pages return a login redirect with an encoded internal `next` path, API routes return exact `401` or `403`, and response bodies have no protected model fields, raw token echo, or signed URLs |
