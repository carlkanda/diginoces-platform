...
| QA-002 | `ROLE-*` | Admin signs in and reaches platform routes | `pending_external_artifact` | `not_classified` | `QAART-pending` | QA lead | AAL2 linked-dev admin smoke exists; external evidence still required. |
...
| QA-020 | `REP-*`; `FILE-*` | Reports, exports, audit logs, retention/archive | `pending_external_artifact` | `not_classified` | `QAART-pending` | Engineering lead | Include audit restriction and file lifecycle evidence. |
...
| QA-027 | `ROLE-*`; `GM-*`; `PAY-*`; `REP-*` | Bride cannot edit groom-only guests or internal/commercial data | `pending_external_artifact` | `not_classified` | `QAART-pending` | QA lead | Exact role-boundary evidence required. |
| QA-028 | `ROLE-*`; `GM-*`; `PAY-*`; `REP-*` | Groom cannot edit bride-only guests or internal/commercial data | `pending_external_artifact` | `not_classified` | `QAART-pending` | QA lead | Exact role-boundary evidence required. |
