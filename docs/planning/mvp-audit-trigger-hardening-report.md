...
- Regression coverage that public-token audit handling does not read RSVP-only fields.
- Regression coverage that payment-exception audit handling does not compare contract-only status values.
- Regression coverage that public guest-message inserts do not read review-only fields.
- Regression coverage that invitation-template audit inserts do not compare invitation-only status values.

## Linked Dev Verification

- Applied all four migrations to the linked dev Supabase project.
