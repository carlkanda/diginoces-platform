...
- Local WSL CodeRabbit full-diff review may fail with `TRPCClientError` even when `coderabbit doctor` passes. When that happens, use scoped directory reviews such as `coderabbit review --agent --base main --dir apps/web/src/lib/auth -c AGENTS.md`, then rely on the hosted CodeRabbit PR review as the full-diff backstop.
